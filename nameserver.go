...
func (a *App) writeCloudLog(r *http.Request, message string, serverity logging.Severity) {
...
//-----------------------------------------------------------------------------
func (a *App) askHandler(w http.ResponseWriter, r *http.Request) {
	who := r.FormValue("who")
	rec, ok := Records[who]
	if !ok {
...
//-----------------------------------------------------------------------------
func (a *App) tellHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || uid == "" {
		...
		a.writeCloudLog(r, fmt.Sprintf("Register %s %s at %s.", who, uid, address), logging.Info)
		fmt.Fprintf(w, uid)
	} else if rec.Uid != uid {
		fmt.Println("Unmatched", who, uid, rec.Uid)
		// writeLog(Config.Log, fmt.Sprintf("%s provided incorrect uid (%s).", who, uid))
		a.writeCloudLog(r, fmt.Sprintf("%s provided incorrect uid (%s).", who, uid), logging.Alert)
		fmt.Fprintf(w, "unmatched")
	} else {
		writeLog(Config.Log, fmt.Sprintf("%s informed new address (%s).", who, address))
		a.writeCloudLog(r, fmt.Sprintf("%s informed new address (%s).", who, address), logging.Info)
		fmt.Fprintf(w, "ok")
	}
}
